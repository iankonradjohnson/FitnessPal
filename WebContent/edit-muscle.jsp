...
<div class="main">
  ...
  <hr>
  <h1><i class="fas fa-swimmer"></i> Edit Muscle </h1>
  
  <div>
  	<img src="./ImageServlet?id=<%=  id  %>&table=muscle" height="400" align="middle" style="display:block; margin:auto; max-width:100%; height:auto; max-height:400px;">
  	<form id="edit-muscle-group" action="EditMuscle?id=<%=  id  %>" method="post" enctype="multipart/form-data">
  		<section class="first">
          	<fieldset class="first_fieldset">
              	<legend>Edit Muscle Group</legend>
              	<label> Muscle Group Name: </label><br>
              	<input type="text" name = "name" size="25" style="width:100%" value = "<%= rs.getString("name") %>" ><br>
                ...
                <label> Muscle Group To Which the Muscle Belongs: </label>
              	<select name="muscle_group" style="width:100%">
              		<% 
    			    ...
    			    while(rs2.next()){
    			    	if(muscle_group_id == rs2.getInt("id")){
    			    		out.print("<option value=\"" + rs2.getString("id") + "\" selected>" + rs2.getString("name") + "</option>");
    			    	} else {
    			    		out.print("<option value=\"" + rs2.getString("id") + "\">" + rs2.getString("name") + "</option>");
    			    	}
    			    }
              		%>
                </select><br>
