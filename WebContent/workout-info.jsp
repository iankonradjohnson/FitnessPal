...
<div class="main">
  ...
  <hr>
  <h1><i class="fas fa-dumbbell"></i> <%= rs.getString("name") %> </h1>
  ...
  <div>
  	...
  	<a href = "weight-lifting.jsp"><%= rs.getString("type") %></a><br><br>
  	<%
  	out.println(writeEquipmentHTML(rs.getString("equipment_ids"), out)+ 
            writeMusclesHTML(rs.getString("muscle_ids"), out));
  	%>
  	<%-- <p>Equipment: <%= rs.getString("equipment") %></p>
  	<p>Muscles Worked: <%= rs.getString("muscles") %></p> --%>
  	<iframe width="640" height="360" src="<%=rs.getString("video")%>" frameborder="0" allowfullscreen></iframe>
