...
public class EditMuscle extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    if(isDuplicate(request, out, id)) {
	    	alert("Dupliacate entry. Item already exists in the database.", "edit-muscle.jsp?id=" + id, out);
	    	return;
	    }
        ...
        if (filePart.getSize() > 0 && filePart.getSize() < 7108864 && filePart.getContentType().contains("image") ) {
        	...
        	alert("Image too large.", "edit-muscle.jsp?id=" + id, out);
        	return;
        	...
        	alert("File not an image.", "edit-muscle.jsp?id=" + id, out);
        	return;
        } else {
        	alert("Something went wrong...", "edit-muscle.jsp?id=" + id, out);
        	return;
        }
	...
	private boolean isDuplicate(HttpServletRequest request, PrintWriter out, int id) {
		...
		String dbURL = "jdbc:mysql://localhost:3306/MyFitnessPal?allowMultiQueries=true";
	    String query = "SELECT * FROM muscle WHERE name = ? AND id != ?";
